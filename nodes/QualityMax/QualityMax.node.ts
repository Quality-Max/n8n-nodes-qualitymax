...
export class QualityMax implements INodeType {
			...
			try {
				...
				// ── K6 ───────────────────────────────────────────────────────────
					...
					if (operation === 'getAll') {
						...
						const scriptId = this.getNodeParameter('k6ScriptId', i) as string;
						responseData = await qualityMaxRequest(this, 'POST', `/k6/run/${scriptId}`);
					} else if (operation === 'checkStatus') {
						const execId = this.getNodeParameter('k6ExecutionId', i) as string;
						responseData = await qualityMaxRequest(this, 'GET', `/k6/status/${execId}`);
					} else if (operation === 'getReport') {
						const execId = this.getNodeParameter('k6ExecutionId', i) as string;
						const format = this.getNodeParameter('reportFormat', i) as string;
